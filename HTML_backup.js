...
const correctSRCAttrValues = (libid, finish = false) => {
  ...
  * c.) korrigálja az elem csoport html kódjában az <img src="" attribútum értékekét,
  * d.) HA egy adott kép nevében NINCS benne a _keep_ betoldás, a STORAGE-ban őrzött képnevet
  *     üres stringre cseréli, magyarán ezt a képet nem kell kimenteni az elem csoporthoz tartozó mappába.
  * e.) a d pontban meghatározott képek esetében az elem csoport HTML kódjának a mappába  történő
  *     kimentésekor HELYŐRZŐ képekre cseréli a képre történő hivatkozásokat.
  */
    return new Promise((resolve, reject) => {
        ...
        for(let i = 0, leng = pics.length; i < leng; i++){
            ...
            let keep_it = pic.indexOf('_keep') > -1;
              ...
              let pic_corrected =  pic.match(/_+\d{3}_\d{3}/) ? pic.replace(/_+\d{3}_\d{3}/, '__' + id) : (pic.substring(0, pic.lastIndexOf('.')) + '__' + id + pic.substr(pic.lastIndexOf('.')));
                ...
                html_in_letter = html_in_letter === "" ? html.replace(reg, pic_corrected) : html_in_letter.replace(reg, pic_corrected);
          ...
          fs.appendFileSync('log_2.html', html_in_letter);
          fs.appendFileSync('log_3.html', html);
