      ...
      if (activeRows.length != prevActiveRows.length) {
        ...
        if (activeRows.length < prevActiveRows.length) {
          ...
          if(rowEnding != null){
            ...
            */
           let match = newActiveRow.match(/(libid_\d{3}_\d{3})[^/]+/);
           newActiveRow = match[0];
           let Lid = match[1];
           rowEnding = rowEnding[1];
           /*
           *  DE! Amennyiben a beírt típus jelölő NEM egyezik a központi style.scss fájlban megadott default, 
           *  az egész kreatívra érvényes típussal (a $template-type változó értéke), akkor át KELL írni az éppen importálás alatt álló elem csoport
           *  saját $this-type értékét, az ő saját _style.scss partial import scss fájljában, a rossz elem csoport változathoz való css 
           *  szabályokat fogják leképezni a háttérben futó kódrészek! (functions.scss fálj)
           */

           let elementGroupHTML = fs.existsSync(`${baseFolder}/element-groups/${newActiveRow}/${rowEnding}/index.css.html`);
              
              if(elementGroupHTML === true){
                    elementGroupHTML = fs.readFileSync(`${baseFolder}/element-groups/${newActiveRow}/${rowEnding}/index.css.html`, 'utf8');
                    makeRowScheme();
                    HTMLAction('import', rows, Lid, elementGroupHTML);
                  }
              else{
                console.log('Invalid path: ' + newActiveRow + '/' + rowEnding);                
              }
          }else{
            /* az aktívvá tett sor végén nem adta meg, az adott elem csoportnak,
            *  melyik változatát szeretné importálni az index.css.html fájlba. 
            */
            let activeRowLibId = newActiveRow.match(/libid_\d{3}_\d{3}/)[0];
            let row_reg = "(\\n[/]{0,2}@import)((?:(?!@import)(?!" + activeRowLibId + ")(?:\\w|\\W))*)(" + activeRowLibId + "[^;]*;)(/\\\*[^*]+\\\*/)?";            
            ...
            console.log('\n>>>>>>\nA ' + activeRowLibId + ' azonosítójú sor végén a 3 kérdőjel helyére írd be, az elem csoport melyik változatát akarod importálni! (TD|DIV|BOTH)\n>>>>>>\n');
